...
def double_conv(in_chan, out_chan):
    ...
    conv = nn.Sequential(
        nn.Conv3d(in_chan, out_chan, kernel_size=(3,3,3),stride=(1,1,1), padding='same'),
        nn.ReLU(inplace=True),
        nn.Conv3d(out_chan, out_chan, kernel_size=(3,3,3),stride=(1,1,1), padding='same'),
        nn.ReLU(inplace=True),
    )
    return conv
...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.max_pooling = nn.MaxPool3d(kernel_size=(3,3,3), stride=(2,2,2))
    ...
    def forward(self, image):
        ...
        print('after conv1')
        x = self.up_trans_2(x)   
        x3 = resize_tensor(x3, x) # Resize x4 to be same size as x
        print('after conv2')
        x = self.up_conv_2(torch.cat([x, x3], 1))
        x = self.up_trans_3(x)
        x2 = resize_tensor(x2, x) # Resize x4 to be same size as x
        x = self.up_conv_3(torch.cat([x, x2], 1))
        print('after conv3')
        x = self.up_trans_4(x)
        x1 = resize_tensor(x1, x) # Resize x4 to be same size as x
